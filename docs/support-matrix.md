...
| Linux native binary (official installer / npm ≥2.1.113) | experimental | 2.1.113 | 2.1.207 linux-x64 live smoke PASS(native 1395, --version 自检 + --help 中文显示) | Linux ELF native binary experimental：`.bun` 节纯 JS 提取/重打包，不需要 node-lief；暂无已发布验证窗口，可识别版本一律安装时本机自验证（provisional），补丁、重打包或启动自检失败自动保留/恢复英文原版。 |
