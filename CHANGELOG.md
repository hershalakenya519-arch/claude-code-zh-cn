...
- 🔴 **修复 117 条翻译在原生二进制上静默失效**：原生二进制（`extract` 出来的 JS）里非 ASCII 一律是**字面转义序列**
  （`·` 写作 `\xB7`、`…` 写作 `\u2026`、emoji 写作代理对，十六进制字母大写），而翻译表里存的是**真实字符**，
  两者匹配不上 → 含 `·`（70 条）、`…`（84 条）、`✓`、`—` 等字符的条目**一条都没生效过**。
  现按源码实测形态补齐转义版兄弟条目，只收录在源码里**验证存在**的形态（117 条），其余丢弃。
  实测命中：**1543 → 1658 条规则**。

### 新增

- **统一设置面板（`/status`、`/config`、`/usage`、`/stats` 打开的同一个五标签面板）全面汉化**，覆盖四个标签：
  - **Status** — 版本 / 会话类型（含「后台任务 · 已连接 / 无人值守 / 交互式」三种取值）/ 工作目录 / 组织 / 邮箱 / 模型 / 通道 / 记忆 / 合规 / 认证令牌 / API 密钥 / API 提供方 / tmux 会话
  - **Usage** — 总花费 / API 时长 / 实际时长 / 代码改动 / 各模型用量 / 当前会话 / 本周（全部模型｜仅 Sonnet）/ % 已用 / 「是什么在消耗你的额度？」及其估算说明 / 常用 skill / 常用 subagent
  - **Stats** — 全部时间 / 最近 7 天 / 最近 30 天 / 「r 换时间段 · ctrl+s 复制」/ ↓ 统计 / ↑ 标签页
  - **Config** — 会话回顾 / Ultracode 关键词触发 / Worktree 基准 ref / 跳过 /copy 选择器 / 自动滚动 / 默认打开 Agent 视图 / 「← 键打开 Agent 视图」/ 消息被标记时切换模型 / 主题
- 翻译表 2073 → **2225 条**；实测命中 **1543 → 1710 条规则**（2.1.222 原生二进制，Linux x64）。

### 改进

- 新增结构化 patch `installSettingsPanelLabelLocalization`：面板行标签统一锚定 `label:"X"` 形态做定向替换。
  这些词裸着遍布全库（实测 `Session` 260 处、`interactive` 178 处、`cwd` 147 处、`Model` 136 处、`Version` 89 处），
  裸加进翻译表必然误伤；锚 `label:` 后每个仅 1-3 处，零误伤。
- **Usage 面板对齐修正**：`Total cost:` 等四行原文按 23 列对齐，中文按 CJK 双宽重算补位空格，避免列错位。
- `CLAUDE.md` 技术要点更正三条踩坑：非 ASCII 双形态、短词不可裸加、patch 输出数字是「命中规则条数」而非「替换位置数」。

### 安全

- `"interactive"` 在源码里另有一处是遥测 `publishContext` 的上报标识，**翻掉会污染上报数据**。
  结构化规则锚定在 `label:"Session kind"` 上，且变量名走通配（不写死混淆名），跨版本稳定。
